...
```{r window-pc, echo=FALSE, fig.align = 'center', out.width='150%', fig.show='hold', fig.cap='Excel Window on a PC.', fig.alt = 'Excel Window as shown on a PC.'}
knitr::include_graphics('window-pc.png')
```

### The Excel Window on a MAC

```{r window-mac, echo=FALSE, fig.align = 'center', out.width='140%', fig.show='hold', fig.cap='Excel Window on a MAC (in Dark Mode).', fig.alt = 'Excel Window as shown on a MAC.'}
knitr::include_graphics('window-mac.png')
```
...
```{r saving-dialog-box-pc, echo=FALSE, fig.align = 'center', out.width='45%', fig.show='hold', fig.cap='Dialog box when saving a workbook on a PC.', fig.alt = 'A dialog box on a PC showing where to enter file name, select the directory and file type.'}
knitr::include_graphics('saving-pc2.png')
```

```{r saving-dialog-box-mac, echo=FALSE, fig.align = 'center', out.width='60%', fig.show='hold', fig.cap='Dialog box when saving a workbook on a MAC.', fig.alt = 'A dialog box on a MAC showing where to enter file name, select the directory and file type.'}
knitr::include_graphics('saving-mac2.png')
```
After using the `Save As` feature, you can use the `Save` command to overwrite your previously saved file with the file in its current state.  After you have saved the Excel file for the first time, you can save the updates during your working session by using the save button on the title toolbar.
...
knitr::include_graphics('saving-options.png')
```
...
knitr::include_graphics('autosave-pc.png')
```
...
3. Select the `AutoSave` or `AutoRecover` info in every check box, and then enter how frequently you want to save the Excel files.
...
knitr::include_graphics('autosave-mac1a.png')
```
...
knitr::include_graphics('autosave-mac1b.png')
```
...
knitr::include_graphics('autosave-mac2.png')
```
...
knitr::include_graphics('toolpak-pc1.png')
```
...
knitr::include_graphics('toolpak-pc2.png')
```
...
knitr::include_graphics('toolpak_mac1.png')
```
...
knitr::include_graphics('toolpak_mac2.png')
```
...
knitr::include_graphics('gridlines-mac.png')
```
...
To select a rectangular block of cells, position the cursor in a corner cell of the block, hold down the left mouse button, drag the cursor to the cell in the opposite corner of the block, and release the button.
...
knitr::include_graphics('cell-selection.png')
```
...
knitr::include_graphics('text-formatting.png')
```
...
knitr::include_graphics('number-formatting.png')
```
...
For every twenty hours of prime-time viewing, the number of ads and the time devoted to ads were recorded. Enter the data as shown in Figure \@ref(fig:entering-data) in your Excel workbook.

```{r entering-data, echo=FALSE, fig.align = 'center', out.width='25%', fig.show='hold', fig.cap='Excel worksheet with data.', fig.alt = 'Excel worksheet with some data typed in.'}
...
The `Clear > Clear Formats` option keeps the content but clears the format. The ` Clear > Clear All` option clears both content and format.
...
The `Sort` and `Filter` buttons on the `Home Menu` shown in Figure \@ref(fig:sorting) sort the data in ascending or descending depending on the selection from the sub-menu. 
...
**Note:** Even if you use the command `Paste` or the shortcut `Ctrl-V` or `Command-V` to paste, you must still press Enter to remove the blinking border around the original cells. To copy one cell or a range of cells to another worksheet or workbook, follow steps (1) and (2) above. For step
...
2.	Move the cursor to the lower right corner of Cell C2. The cursor should change shape to a small black cross (+). Now hold down the left mouse button and drag the `+` down until all the cells in Column C in which you want the calculation done are highlighted.
