...
Excel has few random number generators. The other random functions are `RAND()`, which returns an evenly distributed random real number greater than or equal to 0 and less than 1, and `RANDARRAY(rows,columns,min,max,integer)`, which Returns an array of random numbers. For RANDARRAY, you can specify the number of rows and columns to fill, minimum and maximum values, and whether to return whole numbers or decimal values.
...
knitr::include_graphics('fx-pc.png')
```
...
```{r irandbetween-pc, echo=FALSE, fig.align = 'center', out.width='60%', fig.show='hold', fig.cap='The dialog Box of Insert Function on PC.', fig.alt = 'The dialog box of the Insert Function in Excel on a PC.'}
knitr::include_graphics('randbetween-pc.png')
```
...
6. Adjust the parameters of the formula (top and bottom numbers) as needed.

```{r insert-function-dialog-mac, echo=FALSE, fig.align = 'center', out.width='90%', fig.show='hold', fig.cap='Dialog Box of Insert Function on MAC.', fig.alt = 'The dialog box of the Insert Function in Excel on a MAC.'}
knitr::include_graphics('insert-function-dialog-mac.png')
```
...
knitr::include_graphics('manualcalc-pc.png')
```
...
knitr::include_graphics('manual-calculation1.png')
```
...
knitr::include_graphics('manual-calculation2.png')
```
...
knitr::include_graphics('manual-calculation3.png')
```
...
knitr::include_graphics('recalc-pc.png')
```
**Practice 1:**
...
knitr::include_graphics('style-pc.png')
```

3. Select the `My table has headers` check box in the `Format as Table` dialog box.

```{r format-as-table, echo=FALSE, fig.align = 'center', out.width='70%', fig.show='hold', fig.cap='32: Format As Table dialog box.', fig.alt = 'A screenshot  showing the dialog box from command Format As Table command.'}
knitr::include_graphics('format-table.png')
```

*Remark:* When you select a table by clicking the mouse pointer, the `Table Tools` menu becomes available, and a `Design` tab is displayed. To get a good idea of what you can add to or change in your table, click the `Design` tab, then explore the groups and options provided on this tab.
