...
In this lab, you will use a data set that contains scores received on an MA-336 Exam. Download the data set Lab 3 Exam Scores.xlsx from here:  (**https://github.com/bsosnovski/Intro-Stats-Excel-Lab-Manual/blob/main/Data_Sets/Lab-3-Exam-Scores.xlsx**)
...
**Note:** You can fill in the input range, A2:A33, in the formula count above by manually typing the address of the cells containing the data values. Alternatively, you can select cells that form the Input Range by clicking on A2, hold, and dragging downward until you reach cell A33. Then release.
...
knitr::include_graphics('initial-comp.png')
```

### Class Limits Computations
...
3. Select cell C9 and in the formula bar type `=SUM(C8,$C$5)`. Do not forget the `dollar sign` because the value in C5, that is the width, is a fixed value for all formulas you use here.
...
**Note:** You can also be creative and use a formula to do the work in step for you. There are different ways of entering the upper limits using formulas but that is not being requested from you for this lab. 
...
knitr::include_graphics('class-limits-comp.png')
```
...
2. In the dialog box, first click in the `Input Range` field then type in `$A$2:$A$33`. 
...
5. 5)	In the pop-up window, Check the boxes for `Cumulative Percentage` and `Chart Output`. 
6. Then click `OK`.
...
knitr::include_graphics('histogram-mac.png')
```

After the steps above, Excel presents you with a frequency table and the corresponding graph. Excel counts the frequency of each class based on the *Bins* values you entered in the dialog window for the histogram. Thus, it counts how many values are in the date set (Input Range) up to the upper limits.
...
knitr::include_graphics('raw-histogram.png')
```
...
**Note:** If you don’t delete the row in the table, the graph will contain an extra bar on the right of the graph with a frequency equal to zero. **Be sure to delete the cells and not just delete the contents in those cells.**
...
knitr::include_graphics('delete-more-row.png')
```
...
knitr::include_graphics('adjusting-horizontal-axis-mac.png')
```
...
knitr::include_graphics('adjusting-horizontal-axis-pc.png')
```
...
knitr::include_graphics('adjusted-histogram.png')
```
